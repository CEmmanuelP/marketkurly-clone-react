...
import { shoppingBasketAction } from "../../store/redux";
      ...
      <Item>
        <Thumb>
          <Thumbnail href={`goods/${goods.id}`} src={goods.img}>
            <CartButtonWrapper>
              <CartButton onClick={addCartAction}></CartButton>
            </CartButtonWrapper>
          </Thumbnail>
        </Thumb>
        <Info>
          ...
          <ItemTag>
            {goods.tags.length ? (
              goods.tags.length === 1 ? (
                <ItemTagElement color="#5f0080">{goods.tags[0]}</ItemTagElement>
              ) : (
                <>
                  <ItemTagElement color="#5f0080">
                    {goods.tags[0]}
                  </ItemTagElement>
                  <ItemTagElement color="#666">{goods.tags[1]}</ItemTagElement>
                </>
              )
            ) : (
              ""
